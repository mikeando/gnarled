...
pub fn to_polyline(t: Triangle) -> crate::nbase::polyline::PolyLine<2> {
    PolyLine {
        ps: vec![t.vertices[0], t.vertices[1], t.vertices[2], t.vertices[0]],
    }
}

pub fn to_lines(ts: Vec<Triangle>) -> crate::nbase::lineset::LineSet<2> {
    let mut ls = crate::nbase::lineset::LineSet { lines: vec![] };
    for t in ts {
...
pub fn test_sierpinski() -> Result<(), std::io::Error> {
    use std::io::Write;

    let file_name = "test_sierpinski.svg";
    let mut f = std::fs::File::create(file_name).unwrap();
    writeln!(
        f,
        r#"<svg viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg">"#
