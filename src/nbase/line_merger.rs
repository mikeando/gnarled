...
fn merge<const N:usize>(ls1:&LineSegment<N>, ls2:&LineSegment<N>) -> Option<LineSegment<N>> {
    ...
    if e2<=0.01 {
        ...
        let L = du/u2;
        let x2_new = x0 + u * L;
        Some(LineSegment{ps:[ls1.ps[0], x2_new]})
    } else {
        None
    }
}
...
impl<const N:usize> BinningLineMerger<N> {
    pub async fn run(mut self) -> Result<(),()> {
        while let Some(mut ls) = self.input.recv().await {
            ...
            self.nodes.entry(start_idx).or_default().push(Some((StartOrEnd::Start,idx)));
            self.nodes.entry(end_idx).or_default().push(Some((StartOrEnd::End,idx)));
        }
...
impl<const N:usize> BinningPolyLineMerger<N> {
    pub async fn run(mut self) -> Result<(),()> {
        while let Some(ls) = self.input.recv().await {
            let mut pl = PolyLine{ ps:vec![ls.ps[0], ls.ps[1]]};
